/**
 * Migration runner to add package_name column to dashboard_prp_services table
 * This script adds the missing package_name column to store package names for PRP services
 */

const { Client } = require('pg');
require('dotenv').config();

async function runPackageNameMigration() {
  ...
  try {
    ...
    if (columnCheck.rows[0].exists) {
      console.log('package_name column already exists in dashboard_prp_services table.');
      return;
    }

    console.log('Adding package_name column to dashboard_prp_services table...');
    ...
    try {
      ...
      console.log('✅ Successfully added package_name column to dashboard_prp_services table');
